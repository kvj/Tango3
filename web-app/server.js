...
module.exports.SuperDataProvider = DataProvider;
...
App.prototype.initRest = function() {
	...
	this.app.get('/:code.:type.html', this.htmlLoadApplication.bind(this));
	this.app.get('/:code.:type.manifest', this.htmlGetCache.bind(this));
	this.app.get('/:type', this.htmlGenerateApplication.bind(this));
	...
	this.loadConfig(function (err) {
		if (err) {
			return this.log('Failed to start:', err);
		};
...
App.prototype.loadConfig = function(handler) {
	this.loadFile(CONFIG_FILE, function (err, data) {
		if (err) {
			return handler(err);
		};
		try {
			var loadAnalytics = function (file, config) {
				this.loadFile(file, function (err, data) {
					if (!err) {
						config.analyticsCode = data;
					};
				}.bind(this));
			}.bind(this);
			...
			for (var id in this.config) {
				...
				this.log('Loaded application:', id, config.version);
				config.analyticsCode = '';
				if (config.analytics) {
					// Load analytics code
					loadAnalytics(config.analytics, config);
				};
			}
			return handler(null);
		} catch(e) {
			return handler('Failed to parse JSON');
		}
	}.bind(this));
};
...
App.prototype.htmlLoadApplication = function(req, res) {
	var config = this.config[req.params.type];
	if (!config) {
		// Invalid application type
		res.send(500, 'Application not found');
		return;
	};
	$$.log('Load application:', req.params.type, req.params.code);
	this.loadFile(config.main, function (err, data) {
		if (err) {
		...
		res.send(tmpl.replace('#{manifest}', dev? '': ' manifest="'+req.params.code+'.'+req.params.type+'.manifest"').replace('#{analytics}', dev? '': config.analyticsCode));
	}.bind(this));
};
...
App.prototype.htmlGetCache = function(req, res) {
	...
	var files = config.cache || [];
	var noCacheFiles = config.network || [];
	// this.log('Getting cache:');
	var outp = 'CACHE MANIFEST\n';
	outp += '# rev '+config.version+'\n';
	outp += 'CACHE:\n';
	outp += ''+req.params.code+'.'+req.params.type+'.html\n';
...
App.prototype.htmlGenerateApplication = function(req, res) {
	...
	this.loadFile(config.index, function (err, data) {
		if (err) {
			this.log('Error getting html template:', err);
			res.send(500, 'HTML template not found');
			return;
		...
		res.set('Content-Type', 'text/html');
		var dev = req.url.indexOf('?dev') != -1;
		res.send(tmpl.replace(/#\{code\}/g, id).replace(/#\{type\}/g, req.params.type).replace('#{analytics}', dev? '': config.analyticsCode));
	}.bind(this));
};
