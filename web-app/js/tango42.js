...
NotepadController.prototype.selectPage = function(index) { // Shows page by index
	$$.log('selectPage', index);
	if (!this.pages || !this.pages[index]) { // Invalid index or no pages
		$$.log('Invalid index');
		return false;
	};
	this.showPage(index);
	if (this.pagesVisible) { // If pane hidden - hide pages list
		this.togglePages();
	};
	return true;
};
...
NotepadController.prototype.refreshPagesPane = function() {
	...
	if (this.pages) {
		...
		for (var i = 0; i < this.pages.length; i++) {
			...
			if (i<10) { // Add hotkey
				this.app.el('span', div, {
					'class': 'card_pages_hot'
				}, ''+i);
			};
			this.app.el('span', div, {
			}, item.title);
...
App.prototype.keyHandler = function() {
	...
	document.body.addEventListener('keydown', function (evt) {
		...
		if (!this.sendMessage('editing')) {
			...
			if (code == 73 && e.ctrl) { // Ctrl+i - new page
				panel.createNewItem();
				return stop();
			};
			if (code == 80) { // P - show pages
				panel.togglePages();
				return stop();
			};
			if (e.alt && code>=48 && code<=57) { // Alt+0..9
				panel.selectPage(code-48);
				return stop();
			};
