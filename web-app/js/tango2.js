...
EventEmitter.prototype.on = function(type, handler, top) {//Adds new handler
	...
	for (var i = 0; i < types.length; i++) {
		var t = types[i];
		var arr = [];
		if (!this.events[t]) { //Add empty array
			this.events[t] = arr;
		} else { //Get array
			arr = this.events[t];
		};
		for (var j = 0; j < arr.length; j++) {//Check for duplicate
			if (arr[j] == handler || (arr[j].marker && arr[j].marker == handler.marker)) { //Already here
				arr[j] = handler;
				break;
			};
		};
...
EventEmitter.prototype.off = function(type, handler) {//Removes handler
	...
	var removeFromArray = function (arr, handler) {
		// body...
	...
	if (isFunction(type)) {
		// Remove all types where handler is 
		for(var id in this.events) {
			if (this.events[id]) {
				removeFromArray(this.events[id], type);
			};
		};
		return true;
	};
